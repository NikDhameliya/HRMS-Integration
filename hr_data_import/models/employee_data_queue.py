...
class HRMSEmployeeDataQueue(models.Model):
    """ This model is used to handle the employee data queue."""
    _name = "employee.data.queue"
    _inherit = ["mail.thread", "mail.activity.mixin"]
    _description = "HRMS Synced Employee Data"
    ...
    synced_employee_queue_line_ids = fields.One2many("employee.data.queue.line",
                                                     "synced_employee_queue_id", "Employees")
    ...
    @api.depends('synced_employee_queue_line_ids.common_log_lines_ids')
    def _compute_log_lines(self):
        for line in self:
            line.common_log_lines_ids = line.synced_employee_queue_line_ids.common_log_lines_ids

    @api.depends("synced_employee_queue_line_ids.state")
    def _compute_total_record_count(self):
        ...
        """
        for record in self:
            queue_lines = record.synced_employee_queue_line_ids
    ...
    def create(self, vals):
        """
        This method used to create a sequence of employee queue.
        """
        for val in vals:
            seq = self.env["ir.sequence"].next_by_code("employee.data.queue") or "/"
            val.update({"name": seq or ""})
        return super(HRMSEmployeeDataQueue, self).create(vals)

    @api.model
    def create_employee_queue(self, instance, record_created_from):
        """
        This method used to create a employee queue.
        ...
        return self.create(employee_queue_vals)

    @api.model
    def retrieve_dashboard(self, *args, **kwargs):
        dashboard = self.env['queue.line.dashboard']
        return dashboard.get_data(table='employee.data.queue.line')
