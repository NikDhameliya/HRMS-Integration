...
class HRMSDepartmentDataQueue(models.Model):
    """ This model is used to handle the department data queue."""
    _name = "department.data.queue"
    _inherit = ["mail.thread", "mail.activity.mixin"]
    _description = "HRMS Synced Department Data"
    ...
    synced_department_queue_line_ids = fields.One2many("department.data.queue.line",
                                                     "synced_department_queue_id", "Departments")
    ...
    @api.depends('synced_department_queue_line_ids.common_log_lines_ids')
    def _compute_log_lines(self):
        for line in self:
            line.common_log_lines_ids = line.synced_department_queue_line_ids.common_log_lines_ids

    @api.depends("synced_department_queue_line_ids.state")
    def _compute_total_record_count(self):
        ...
        """
        for record in self:
            queue_lines = record.synced_department_queue_line_ids
    ...
    def create(self, vals):
        """
        This method used to create a sequence of department queue.
        """
        for val in vals:
            seq = self.env["ir.sequence"].next_by_code("department.data.queue") or "/"
            val.update({"name": seq or ""})
        return super(HRMSDepartmentDataQueue, self).create(vals)

    @api.model
    def create_department_queue(self, instance, record_created_from):
        """
        This method used to create a department queue.
